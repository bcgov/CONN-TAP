#!/usr/bin/env python3
"""Run every Rogers NGTA cellular validation and export the results to one Excel workbook.
...
legacy Python/Excel report (Duplicates, Null_BGE, Null_SUB-BGE, Missing_BGEs,
Missing_SUB_BGEs, Mapping_Issues, Mapping_Summary, Unknown_SUB-BGE, New_BGEs,
...
-----
  ...
  python scripts/validation/rogers/cellular/run_validations.py

  # scope to one statement month (any date within it); omit to scan all months
  python scripts/validation/rogers/cellular/run_validations.py --month 2026-06

  # point at a specific DSN / output file, and skip recreating the SQL functions
  python scripts/validation/rogers/cellular/run_validations.py --dsn "$DATABASE_URL" --no-apply

Each validation function takes an optional month (a date within the target month); the
detection check treats that month as "current" for its month-over-month comparison. When
--month is omitted the checks scan all months (detection uses the newest month).
...
# scripts/validation/rogers/cellular -> scripts
...
DDL_FILES = [
    HERE.parent / "helpers" / "_shared.sql",
...
VALIDATIONS = [
    ("Duplicate Rows",
     "Every row in a business-key duplicate group "
     "(invoice_date, company_code, subscriber_no, pre/post-tax amount).",
     "SELECT * FROM raw_data.rogers_cellular_duplicates(%s)"),
    ("Null BGE",
     "Rows whose raw BGE is null or blank.",
     "SELECT * FROM raw_data.rogers_cellular_null_bge(%s)"),
    ("Null SUB-BGE",
     "Rows whose raw SUB-BGE is null or blank.",
     "SELECT * FROM raw_data.rogers_cellular_null_sub_bge(%s)"),
    ("Missing BGEs",
     "Expected canonical BGEs (from the seeds) absent entirely from the report.",
     "SELECT * FROM raw_data.rogers_cellular_missing_bges(%s)"),
    ("Missing SUB-BGEs",
     "Expected SUB-BGEs (from the seeds) absent entirely from the report, with related BGE.",
     "SELECT * FROM raw_data.rogers_cellular_missing_sub_bges(%s)"),
    ("Mapping Issues",
     "Rows where the reported BGE differs from the BGE expected by its SUB-BGE.",
     "SELECT * FROM raw_data.rogers_cellular_mapping_issues(%s)"),
    ("Mapping Summary",
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=None,
                        help="Output .xlsx path (default: "
                             "scripts/rogers_cellular_validation_report[_YYYY-MM].xlsx)")
    ...
    if args.output is not None:
        ...
        output = SCRIPTS_DIR / f"rogers_cellular_validation_report{suffix}.xlsx"
    ...
    with psycopg.connect(args.dsn) as conn:
        ...
        if month is not None:
            ...
            results.append((
                "Spend Comparison",
                "Month-over-month cellular spend by BGE per category "
                "(hardware/plans/total): current, previous, difference, and a >50% change flag.",
                df,
            ))
        else:
