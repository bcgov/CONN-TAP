...
BEGIN
    ...
    */
    ...
    FOREACH v_option IN ARRAY p_options LOOP
        ...
        WHERE reporting._norm_key(e.key)
              = reporting._norm_key(v_option)
        LIMIT 1;
...
BEGIN
    ...
    /*
      ...
      If BILLINGDATE is already date/timestamp
      in the table, to_jsonb passes it as text.
    */
...
(
    ...
    WHERE coalesce(
        trim(monthly_report_service_id),
        ''
    ) <> ''
),
...
/*
  Rows from the monthly report where
...
(
    SELECT
        ...
        'Missing Service ID from Report'::text
            AS match_status_service_id,

        NULL::numeric
            AS monthly_fixed_fee_from_price_book,
        ...
        billed_amount_numeric
            AS billed_amount_pre_tax,

        NULL::numeric
            AS difference_rate_minus_monthly_fixed_fee
...
/*
  Rows where Service ID exists in
  the monthly report and can be compared
...
(
    ...
    LEFT JOIN price_clean pc
        ...
        ON r.normalized_service_id
           = pc.normalized_service_id
),


/*
  Combine normal comparisons with
  rows where the report Service ID is missing.
*/
...
SELECT
    ...
    price_book_service_id
        AS "Price Book Service ID",

    monthly_report_service_id
        AS "Monthly Report Service ID",

    product_line
        AS "Product Line",

    charge_description
        AS "Charge Description",

    match_status_service_id
        AS "Match Status (Service ID)",

    monthly_fixed_fee_from_price_book
        AS "Monthly Fixed Fee (from the Price Book)",
    ...
    billed_amount_pre_tax
        AS "Billed Amount (Pre-Tax)",

    difference_rate_minus_monthly_fixed_fee
        AS "Difference (Rate - Monthly Fixed Fee)"
