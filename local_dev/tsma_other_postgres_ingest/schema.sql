-- TSMA Other (managed security / router / WLAN–Wi‑Fi) — raw landing. Run:
...
-- Same wireline-shaped columns as tsma_wireline (plan: managed_security / router / wlan_wifi).
...
CREATE TABLE IF NOT EXISTS tsma_other_managed_wlan_wifi (
  raw_id                    bigserial PRIMARY KEY,
  ingestion_run_id          bigint NOT NULL REFERENCES tsma_other_ingestion_run (tsma_other_ingestion_run_id),
  month_id                  integer,
  month_start_dt            date,
...
CREATE INDEX IF NOT EXISTS idx_tsma_other_managed_wlan_wifi_ingestion
  ON tsma_other_managed_wlan_wifi (ingestion_run_id);
