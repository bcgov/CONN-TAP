...
Mirrors telus/ingest.py: one workbook -> one pricebook_ingestion_run row
...
from rogers.catalogues import resolve_book
from rogers.excel import parse_workbook
...
def process_file(
    ...
    *,
    ...
    table_columns: dict[str, tuple[str, ...]] = {spec.table_name: spec.columns for spec in book.sheets}
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                """,
                ("rogers", book.book_code, path.resolve().as_uri(), source_period),
            )
