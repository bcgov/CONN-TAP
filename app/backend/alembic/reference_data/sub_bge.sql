...
INSERT INTO reference_data.sub_bge (code, name, bge_id, entity_type, parent_sub_bge_id, onboarded, cellular, data, voice) VALUES
    ...
    ('LEGISLATIVE ASSEMBLY',                                'Legislative Assembly',                                (SELECT id FROM reference_data.bge WHERE code = 'Gov BC'), 'service_designee', (SELECT id FROM reference_data.sub_bge WHERE code = 'Citizens Services'), true,  'Self', 'Self', 'CSBC'),
