...
# health. Avoids deadlocks: EBS CSI + IMDS disabled needs Pod Identity, but
# Pod Identity was previously blocked behind depends_on = [module.eks] while
# module.eks waited for the EBS add-on to go ACTIVE.
resource "terraform_data" "eks_control_plane_ready" {
  input = module.eks.cluster_arn
}

# Private EC2 for SSM Session Manager only; use port forwarding to reach Superset RDS on 5432.
# Example (after RDS endpoint is known):
#   aws ssm start-session --target "$(terraform output -raw ssm_bastion_instance_id)" \
#     --document-name AWS-StartPortForwardingSessionToRemoteHost \
#     --parameters "{\"host\":[\"$(terraform output -raw superset_rds_address)\"],\"portNumber\":[\"5432\"],\"localPortNumber\":[\"5432\"]}"
...
locals {
  # Second values layer for secrets only — avoids helm_release set_sensitive Set inconsistency when
  # values are finalized mid-apply (hashicorp/helm #737-class issues).
